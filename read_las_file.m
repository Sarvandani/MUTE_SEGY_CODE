...
%      if buffer(ia) ~= ' '
    ...
    if strcmpi(kw,'COMP')
      wlog.company = strim(buffer(ia:ind));  
    end	
	      
    if strcmpi(kw,'WELL')
      wlog.wellname = strim(buffer(ia:ind));
    end

    if strcmpi(kw,'FLD')
      wlog.field = strim(buffer(ia:ind));  
    end	
	      
    if strcmpi(kw,'LOC')
      temp=strim(buffer(ia:ind));
...
if strcmpi(buffer(1:2),'~p') 
  ...
  while buffer(1:1) ~= '~' 
    ...
    if ~strcmpi(buffer(1),'#') 
      ...
      ind=ind(1);
      parameter=lower(strim(buffer(1:ind)));
      ind1=find(buffer == ':');
      ...
      value=strim(buffer(ind+nu+2:ind1-1));
      temp=strim(buffer(ind1+1:end));
      if ~isempty(temp)
...
%       curve_info(2,nlogs)={sscanf(buffer(ind+2:ind1-1),'%s',1)};
        temp=strim(buffer(ind+2:ind1-1));
        idx=findstr(temp,'  ');
