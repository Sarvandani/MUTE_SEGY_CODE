function filter=s_wiener_filter(from,to,varargin)
% Function computes the filter that matches (filters that match) the first
% input data set to the second input data set.
%
% Written by: E. R.: October 2001
% Last updated: August 3, 2006: bug fix
%
%             filter=s_wiener_filter(from,to,varargin)
% INPUT
...
%             these crosscorrelations for option "tracewise". To suppress 
%             cresting this header set {'header',[]}
...
if n2 > n1-nf   % Number of samples of target >= number of samples of original
   ...
   alert(' ... not yet sufficiently tested for multi-trace filters')
