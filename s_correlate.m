function corr=s_correlate(seis1,seis2,varargin)
...
% 
% Written by:  E. R.: November 21, 2000
% Last updated: May 8, 2006: Change default of input argument "option". 
%
...
%    'lags'  two numbers representing the first and last shift of seis2 with respect to seis1
%          for which the correlation is to be computed. They become the times of the first and 
%          last sample, respectively, of the output data set and must be in integer multiple
%          of the sample interval of the seismic input data sets (they are changed to an
...
%          'no'      ==> no normalization
%          'traces'  ==> traces of "seis1" and "seis2" are normalized to have L2 norm 1
%          'segment' ==> trace segments correlated are normalized individually; in this case 
%          the lags must be within the range defined by the default.                    
%          Default: {'normalize','traces'}  
%   
%    'option'  describes the type of operation performed. Possible values are: 
%          'all' which performs correlates every trace of "seis1" with every trace of "seis2".
%                The output traces represent "seis1" correlated with the first trace of "seis2"
%                followed by "seis1" correlated with the second trace of "seis2", etc.
%          'corresponding' which correlates each trace of "seis1" with the corresponding trace
%                of "seis2". Thus "seis1" and "seis2" must have the same number of traces.
...
if ~isstruct(seis1) | ~isstruct(seis2)
...
if strcmpi(param.option,'all')    % Put trace numbers into headers
   ...
   corr=s_header(corr,'add',param.headers{1},headerv1,'na',['Trace number of "',ds1,'"']);
   corr=s_header(corr,'add',param.headers{2},headerv2,'na',['Trace number of "',ds2,'"']);
