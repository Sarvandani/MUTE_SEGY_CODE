...
%       
% Written by: E. R.: December 26, 2000
% Last updated: April 14, 2006: temporarily suspend warning about NaN's in "interp1".
%
...
%     
% OUTPUT 
% wlog  combination of the two input logs
   ...
   if isempty(index)
      ...
      for ii=index;
         ...
         if max(abs(curve1-curve2)) <= 1.0e-3*max(curve1)
            ...
            s_header(wlog,'add',[mnem,'_no_1'],curve2,wlog2.curve_info{ii,2},wlog2.curve_info{ii,3})
         end
      end
   end
...
%  if ~isempty(index)
  if any(isnan(wlog.curves(:,2:end)))
     wlog.null=NaN;
  end
end  
