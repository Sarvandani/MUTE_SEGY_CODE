...
% Unless keyword 'times' is {'min','max'} and keyword 'headers' is 'i'
% some seismic samples and/or header values of "seisout" may not be defined by the 
% input data.
% If seis1.null and seis2.null exist and are the same this null value is used 
% for undefined data
% If only one of the data sets has a null value defined it is used. 
% If neither of the data sets has a null value defined NaN is used.
%
% Written by: E. R.: January 28, 2000
% Last updated: April 8, 2006: handle initial case of successivley appending
%                              datasets (convenience feature)
%
...
%         are ignored). This is intended to make it simpler to to build a
%         multi-trace dataset in a loop.
% seis2   seismic structure
...
if seis1.step ~= seis2.step
  error([' Input data sets have different sample intervals (', ...
     num2str(seis2.step),' ',seis1.units,')'])
end
...
if isfield(seis1,'null') & isfield(seis2,'null')
   if ~isnan(seis1.null) & ~isnan(seis2.null)
...
if strcmpi(htype,'u')
   if isempty(iheaders)
      ...
      [dummy,idx1]=setdiff(headers1(:,1),iheaders(:,1));
      [dummy,idx2]=setdiff(headers2(:,1),iheaders(:,1));
   end
