...
% Last updated: February 16, 2006: save curve handle even if there is only one curve
%
...
if ~istype(seismic,'seismic')
   error(' First input argument must be a seismic')
end
...
if strcmpi(param.figure,'new')
   ...
   if isempty(param.title)
     title(['Headers of seismic data set "', ...
         strrep(inputname(1),'_','\_'),'"'],'FontSize',14,'Color','r')
   else
     title(param.title,'FontSize',14,'Color','r')
   end
end
hold on
