...
% Updated: December 24, 2005: use "timeStamp" and "mysupplot"
%
...
%             Default: {'linewidth',1}
%      'linkedzoom'   Only used if two or more curves are plotted. In this
%             case it specifies if the if a zoom should be the same on each 
%             subplot. Possible values are 'yes' and 'no'. If 'yes' is chosen
%             the legend will be absent. To preserve the legend use 'no'.
%             Default: {linkedzoom','yes'}
...
%         l_litho_plot(wlog,{'curves','DTp','rho'},{'lithos','shale','sand'}, ...
%                           {'depths',6000,8000});
...
if ~isempty(param.title)
   if ncurves > 1
      mysuptitle(mnem2tex(param.title),{'yloc',1.03})
...
%               index=index0(find(wlog.curves(index0,idx0)));
 ...
 %              end
	 ...
	 initiate_2d_tracking(l_info(wlog,param.curves{ii}),wlog.curve_info(1,:));
      end
